...
for(var rowIdx=0; rowIdx<fieldHeight+1; ++rowIdx) {
	...
	for(var columnIdx=0; columnIdx<fieldWidth; ++columnIdx) emptyRow[columnIdx] = false;
...
function createLine() {
	...
	for(var i=fieldWidth-1; i>=0; --i) {
		...
		var targetBox = game.add.sprite(x, y, 'boxCollection' + Number(colorOn), frameIdx);
...
function checkSimilarities(posX, posY, frame) {
	if(logging)
		console.log("checkSimilarities(" + posX + "," + posY + "," + frame + ")");
	...
	if(similarBoxCount >= 2){
		...
		var fallingBoxes = cleanUp(similarBoxCount);
	...
	if(logging)
		console.log("/checkSimilarities" + " Tmp: " + shootCounterTmp + "; Count: " + shootCounter);
}
...
function cleanUp(erasedBoxes){
    ...
    var newPoints = Math.floor((erasedBoxes+1)/3);
	points += newPoints;
	coin_Points += newPoints;
	updatePoints();
	...
	// create new level and increase colorCount if tutorial
	if(stonesLeft<=0){
...
function createFallingBox(posX, posY) {
	...
	var fallingBox = game.add.sprite(x, y, 'boxCollection' + Number(colorOn), targets_Group.getAt(idx).frame);
